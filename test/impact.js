...
contract('Singe impactRegistry donation', function(accounts) {
  ...
  var master = accounts[2];
  var impactRegistry;

  it("should attach master contract", function(done) {
    ImpactRegistry.deployed().then(function(instance) {
      impactRegistry = instance;
      return impactRegistry.setMasterContract(master, {from: main});
    }).then(function() {
      return impactRegistry.masterContract();
    }).then(function(address) {
      return assert.equal(address, master, "Master contract hasn't been set up correctly");
    })
      .then(done)
      .catch(done);
  });
  ...
  it("should register impactRegistry from donor1", function (done) {
    impactRegistry.registerDonation(donor1, 10, {from: master}).then(function () {
      return impactRegistry.getBalance.call(donor1, {from: main})
    }).then(function (balance) {
      return assert.equal(balance.valueOf(), 10, "10 wasn't in registry after donation");
    })
      .then(done)
      .catch(done);
  });
  ...
  it("should register outcome", function (done) {
    impactRegistry.registerOutcome("outcome", 10, {from: master}).then(function () {
  ...
  it("should link impactRegistry", function (done) {
    impactRegistry.linkImpact("outcome", {from: main}).then(function () {
      ...
      assert.equal(linked.valueOf(), 10, "Should link all of the impactRegistry");
